...
fn pass_generator_normal(
    padding: Option<&str>,
    wordlength: Option<i8>,
    wordcount: Option<i8>,
...
fn generate_phrase(wl: &i8, wc: &i8, cp: &str) -> String {
    let padding_index: usize =
        StdRng::from_entropy().gen_range(0..PADDING.len().try_into().unwrap());
...
fn query_validator<'a>(
    padding: Option<&'a str>,
    wordlength: Option<i8>,
    wordcount: Option<i8>,
    capitalization: Option<&'a str>,
) -> (&'a str, i8, i8, &'a str) {
    ...
    let v_wordlength: i8 = if v_wordlength > WORD_LENGTH_MAX || v_wordlength < WORD_LENGTH_MIN {
        WORD_LENGTH_DEFAULT
    } else {
        v_wordlength
    };
    ...
    let v_wordcount: i8 = if v_wordcount > WORD_COUNT_MAX || v_wordcount < WORD_COUNT_MIN {
        WORD_COUNT_DEFAULT
    } else {
        v_wordcount
    };
    ...
    let v_capitalization: &str = if CAPITALIZATIONS.contains(&v_capitalization) {
        v_capitalization
    } else {
        CAPITALIZATION_DEFAULT
    };
...
fn rocket() -> _ {
    ...
    let buf =
        BufReader::new(fs::File::open("./share/dict_en.txt").expect("dict_en.txt not found!"));
